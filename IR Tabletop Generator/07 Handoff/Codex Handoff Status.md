...
The AAR-only print/export slice is live and verified. Keep churn low while AdSense review is pending. Reasonable later slices remain: deliberate URL hygiene for clean URLs/canonicals/sitemap after AdSense or explicit approval, or a small landing URL cleanup so bare `/` stays visually clean until the user changes state.
