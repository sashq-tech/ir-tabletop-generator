...
- 2026-07-18 Search Console sitemap/canonical cleanup pass began after Sean reported a concrete defect: the live sitemap listed eight `.html` URLs that returned 308 redirects to extensionless canonical URLs, and Search Console reported 10 redirecting variants overall. Scope is limited to URL hygiene: make sitemap `<loc>` values, canonical metadata, Open Graph URLs, structured-data URLs, and static internal hrefs point at extensionless final routes while preserving old `.html` compatibility redirects and leaving landing/product content, ads, DNS, and security unchanged.

## Next Recommended Step

The AAR-only print/export slice is live and verified. The sitemap/canonical cleanup is the active 2026-07-18 Search Console defect fix. After it is published and live-verified, keep churn low while AdSense review is pending. A reasonable later slice remains a small landing URL cleanup so bare `/` stays visually clean until the user changes state.
